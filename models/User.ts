...
const UserSchema = new Schema(
  {
    ...
    /*
      ...
      이 앱은 이메일로만 로그인하지만, 컬렉션은 여섯 앱이 공유하고
      전화번호만 있는 계정이 아직 남아 있다. 여기서 필수로 걸면 그 계정들의
      `user.save()` 가 `User validation failed: email` 로 터진다 —
      다른 앱이 `lastLoginAt` 만 갱신하려 해도 마찬가지다.
    ...
    */
     ...
     * ── 이메일 인증 (여섯 앱 공용) ──
     *
     * `emailVerified` 는 원래 이 앱만 쓰던 필드다. 포털이 인증을 하게 되면서
     * 공용으로 올려 썼다 — 값의 의미는 그대로라 이 앱의 동작은 바뀌지 않는다.
     *
...
/**
 * `users` 컬렉션은 앱 DB(`hamhibokka`)가 아니라
 * `MONGO_USER_DB`(기본 `user`) DB에 있습니다. 반드시 `connectDB()` 완료 후 호출하세요.
 */
