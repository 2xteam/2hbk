...
import { loadSession, loadSessionToken, type SessionUser } from "@/lib/session";
...
export function useSession(): SessionState {
  ...
  useEffect(() => {
    ...
    // 서명 토큰이 없는 세션은 API를 부를 수 없다. 로그인하지 않은 것과 같게 다룬다
    if (user && loadSessionToken()) setState({ status: "signed-in", user });
