 ...
 * 이 앱의 데이터가 들어갈 DB 이름.
 *
 * URI 뒤에 붙은 경로에 의존하지 않고 **명시적으로 지정**한다.
 * 다른 앱의 연결 문자열을 그대로 붙여 넣으면 URI 경로가 다른 DB를 가리켜
 * 조용히 엉뚱한 DB에 쓰기 때문이다. (2026-09-02 FitLog가 `math` DB에
 * measurements 를 쓰던 사고 — my-obsidian-vault → 40-Infra/MongoDB Atlas.md)
 */
const MONGODB_DB = (process.env.MONGO_DB ?? "hamhibokka").trim() || "hamhibokka";
